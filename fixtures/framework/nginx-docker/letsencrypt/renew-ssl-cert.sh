...
docker stop <dir>_http-redirect_1
...
function cleanup {
  docker-compose down
  docker start <dir>_http-redirect_1
}
...
if [[ "${DRY_RUN}" = "true" ]]; then
  ...
  echo "Copying key and cert to the <dir> nginx ssl dir..."
  set -x
  ...
  set -x
  sudo docker cp --follow-link ${cert_path} <dir>_nginx_1:/etc/ssl/server.pem
  sudo docker cp --follow-link ${key_path} <dir>_nginx_1:/etc/ssl/server.key
  ...
  set -x
  sudo docker exec <dir>_nginx_1 nginx -s reload
